...
def test_full_pipeline_flow(mock_post, pipeline, temp_python_file):
    ...
    assert result.final_content == "def core_logic(): return True"
    assert len(result.history) == 2
    assert result.history[0].details["type"] == "optimization"
    assert result.history[1].details["type"] == "compression"
